...
class Trader:
    ...
    def __init__(self) -> None:
        ...
        self.data = RollingList(max_length = 15)
...
############################################
    def starfruit_strategy(self, state : TradingState) -> List[Order]:
        ...
        """
        orders = []
        position_starfruit = self.self.get_position(STARFRUIT, state)
        average_price = self.get_mid_price(STARFRUIT, state)

        self.data.add_point(average_price)

        # Sample stock price data (replace this with your own data)
        prices = self.data.get_data()
    ...
    def buy(self, product, quantity, state : TradingState) -> List[Order]:
        orders = []
        POSITION[product] = self.get_position(product, state)
        bid_volume = min(self.position_limit[product] - POSITION[product], quantity)
        asks_below_default = [ask for ask in state.order_depths[product].sell_orders if ask < self.get_mid_price(product, state)]
        asks_below_default = sorted(asks_below_default)
        for ask in asks_below_default:
           quantity = min(state.order_depths[product].sell_orders[ask], bid_volume)
           orders.append(Order(product, ask, quantity))
           bid_volume -= quantity
        return orders
    
    def sell(self, product, quantity, state : TradingState) -> List[Order]:
        orders = []
        POSITION[product] = self.get_position(product, state)
        ask_volume = max(-self.position_limit[product] - POSITION[product], -quantity)
        bids_above_default = [bid for bid in state.order_depths[product].buy_orders if bid > self.get_mid_price(product, state)]
        bids_above_default = sorted(bids_above_default, reverse=True)
        for bid in bids_above_default:
            quantity = max(state.order_depths[product].buy_orders[bid], ask_volume)
    ...
    def amethysts_strategy(self, state : TradingState) -> List[Order]:
        ...
        buy_room = POSITION_LIMIT[AMETHYSTS] - position_starfruit
        sell_room = -POSITION_LIMIT[AMETHYSTS] + position_starfruit
        ...
        if 9995 in asks_below_default or 9996 in asks_below_default:
          orders.append(Order(AMETHYSTS, 9996, bid_volume))
        else:
          quantity = min(20, bid_volume)
          orders.append(Order(AMETHYSTS, 9998, quantity))
        if 10004 in bids_above_default or 10005 in bids_above_default:
          orders.append(Order(AMETHYSTS, 10004, ask_volume))
        else:
          quantity = max(ask_volume, -20)
          orders.append(Order(AMETHYSTS, 10002, quantity))
...
class RollingList:
    def __init__(self, max_length):
        ...
        self.data = pd.DataFrame(columns=['value'])
