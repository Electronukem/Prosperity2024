...
############################################
    def starfruit_strategy(self, state : TradingState) -> List[Order]:
        ...
        """
        # Percent change: threshold to determine if change in trend pattern
        change_percent = 0.003
        POSITION[STARFRUIT] = self.get_position(STARFRUIT, state) + POSITION[STARFRUIT]

        # position_starfruit = self.get_position(STARFRUIT, state)
        position_starfruit = POSITION[STARFRUIT]
        ...
        if len(state.order_depths[STARFRUIT].sell_orders) != 0:
            ...
            change = abs((best_ask - self.ema_prices[STARFRUIT])/self.ema_prices[STARFRUIT])
            if(self.zig_up == True and best_ask < self.ema_prices[STARFRUIT]):
                if(change > change_percent):
                    self.zig_up = False
                    change = True
            elif(self.zig_up == False and best_ask > self.ema_prices[STARFRUIT]):
                if(change > change_percent):
                    self.zig_up = True
                    change = True
        ...
        if (self.zig_up == True and change == True):
            for ask in state.order_depths[STARFRUIT].sell_orders:
                space = bid_volume - abs(state.order_depths[STARFRUIT].sell_orders[ask])
                if(space < POSITION_LIMIT[STARFRUIT]):
                    orders.append(Order(STARFRUIT, ask, -space))
                    bid_volume -= space
                else:
                    orders.append(Order(STARFRUIT, ask, -state.order_depths[STARFRUIT].sell_orders[ask]))
                    bid_volume -= state.order_depths[STARFRUIT].sell_orders[ask]


        elif (self.zig_up == False and change == True):
            for bid in state.order_depths[STARFRUIT].sell_orders:
                space = ask_volume - abs(state.order_depths[STARFRUIT].buy_orders[bid])
                if(space > 0):
                    orders.append(Order(STARFRUIT, bid, space))
                    ask_volume = -space
                else:
                    orders.append(Order(STARFRUIT, ask, state.order_depths[STARFRUIT].buy_orders[bid]))
                    bid_volume -= state.order_depths[STARFRUIT].buy_orders[bid]

        elif (self.zig_up == True and position_starfruit < POSITION_LIMIT[STARFRUIT]):
            for ask in state.order_depths[STARFRUIT].sell_orders:
                space = bid_volume - abs(state.order_depths[STARFRUIT].sell_orders[ask])
                if(space < POSITION_LIMIT[STARFRUIT]):
                    orders.append(Order(STARFRUIT, ask, -space))
        ...
        elif (self.zig_up == False and position_starfruit > 0):
            for bid in state.order_depths[STARFRUIT].sell_orders:
                space = ask_volume - abs(state.order_depths[STARFRUIT].buy_orders[bid])
                if(space > 0):
                    orders.append(Order(STARFRUIT, bid, space))
    ...
    def amethysts_strategy(self, state : TradingState) -> List[Order]:
        ...
        position_amethysts = self.get_position(AMETHYSTS, state)
        orders = []
        #POSITION[AMETHYSTS] = self.get_position(AMETHYSTS, state) + POSITION[AMETHYSTS]
        #position_starfruit = POSITION[AMETHYSTS]
        buy_room = POSITION_LIMIT[AMETHYSTS] - position_amethysts
        sell_room = -POSITION_LIMIT[AMETHYSTS] + position_amethysts

        bids_above_default = [bid for bid in state.order_depths[AMETHYSTS].buy_orders if bid > DEFAULT_PRICES[AMETHYSTS]]
        asks_below_default = [ask for ask in state.order_depths[AMETHYSTS].sell_orders if ask < DEFAULT_PRICES[AMETHYSTS]]

        #Process if there's order to buy
        if (len(asks_below_default) > 0 and buy_room > 0): 
            for ask in asks_below_default:
                if(ask == 9998):
                    # buy 10
                    if(buy_room > 10):
                        buy_room = 10
                    if(buy_room > abs(state.order_depths[AMETHYSTS].sell_orders[ask])):
                        orders.append(Order(product, ask, state.order_depths[AMETHYSTS].buy_orders[bid]))
                        buy_room -= state.order_depths[AMETHYSTS].buy_orders[bid]]
                    else:
                        orders.append(Order(product, ask, -buy_room))
                        buy_room -= buy_room
                else:
                    # buy till full
                    if(buy_room > abs(state.order_depths[AMETHYSTS].sell_orders[ask])):
        ...
        # Selling
        if len(bids_above_default) != 0:
            for bid in bids_above_default:
                if(bid == 10002):
                    # sell 10
                    if(sell_room > 10):
                        sell_room = 10
                    if(sell_room > abs(state.order_depths[AMETHYSTS].sell_orders[ask])):
                        orders.append(Order(product, bid, -state.order_depths[AMETHYSTS].sell_orders[ask]))
                        sell_room -= abs(state.order_depths[AMETHYSTS].sell_orders[ask])
                    else:
                        orders.append(Order(product, bid, -sell_room))
                        sell_room = 0
                else:
                    # sell till full
                    if(buy_room > abs(state.order_depths[AMETHYSTS].sell_orders[bid])):
                        orders.append(Order(product, bid, -state.order_depths[AMETHYSTS].sell_orders[bid]))
                        buy_room -= abs(state.order_depths[AMETHYSTS].sell_orders[bid])
                    else:
                        orders.append(Order(product, ask, -buy_room))
                        buy_room = 0
        return orders
